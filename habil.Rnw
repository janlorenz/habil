...
\usepackage{amsfonts, amsmath, eurosym}
\usepackage{authblk}
\usepackage{paralist} 
\usepackage{tikz}
\usepackage{booktabs}
...
# Recompute lengthy stuff? (Warning: Takes hours!)
compute_OPDboundedconfidence <- FALSE # Warning: Computation takes hours!
compute_OPDias <- FALSE # Warning: Computation takes hours!
...
\section{Tipping diffusivity and why more links can lead to less consensus} 
...
The analog process $(\bar{x}(t))_t$ instead is a time-dependent Markov process, because $\bar{x}(t+1) = \bar{x}(t) + 1$ with probability $p(t,\bar{x}(t),q)$ and $\bar{x}(t+1) = \bar{x}(t)$ otherwise. Here, the transition probabilities only depned on the current state and the time step. 
...
\section{Average and deviate -- how movie rating histograms take shape} 
...
\section{How clusters in ideology space emerge through opinion dynamics under bounded confidence} 
...
\textbf{Open-minded agents} (Fig.~\ref{fig:reconsider}C $\varepsilon=0.3$): Under an intermediate probability of reconsideration ($p=0.09$) the opinion landscape shows a large central peak as observed empirically, also small peaks at the extremes match the emprical landscapes, although empirical extremal peaks lie more pronounced at 0 and 10 and not on 0,1 respectively 9,10 as in the model. The weak point here is that empirically we observe much more mass on the bins close to the center. 
...
\section{Systemic effects in opinion dynamics and opinion pattern diagrams}
...
ggplot(M,aes(fac,y)) + annotate("rect", xmin = 0.1, xmax = 3.02, ymin = -1, ymax = 1, fill="#3288BD") +
  ...
  labs(y="y (stable opinions)", x=expression(
    Omega~"(Diffusivity measured in"~Delta~"with"~Omega[X]==0.25*Omega~"and"~Omega[0]==Omega~")")) +
...
# Computation takes several HOURS!!! (when R/bc_opinion_pattern_diagram_m0.1 has to be built anew)
if (compute_OPDboundedconfidence) {makeM(m = 0.1, P_init, E, prec=10^-14)}
ggplot(getR(0.1), aes(confidencebound, opinion)) + 
...
Growth is fundamental to all living organisms. 
 ...
 Macroeconomic growth can be defined analogously. % TODO Check 
 ...
 The three   two contributions to systemic effects on societal growth. 

 First, \textcite{Koenig.Lorenz.ea2016Innovationvsimitation} show how innovation together with imitation of technology optimizes productivity growth in an economy of independent firms. 
 The second contribution \parencite{Lorenz.Paetzel.ea2013Redistributionspursgrowth}, shows how redistribution of wealth through proportional taxation and redistribution as equal lump-sums can spur growth. 
 
 
 Such growth through redistribution is triggered when individual growth is proportional but rates of growth (or decline when below one) are partly stochastic. 
 ...
 An experimental study \parencite{Lorenz.Paetzel.ea2016JustDontCall} on framing of collective decisions about redistribution shows an important aspect of the answer to this question There is a higher propensity to high tax rates  
\bigskip

\section{How innovation and imitation drive growth of productivity.} \textcite{Koenig.Lorenz.ea2016Innovationvsimitation} show how innovation together with imitation of technology maximized productivity growth in an economy of firms. The theory reproduces the empirical fact that the productivity distribution of domestic firms in existing economies are travelling waves with left and right tails following power laws. 
...
\begin{figure}[tbp]
 \centering%
 \includegraphics[width=0.6\textwidth]{figs/fig_examplesQ-eps-converted-to}
 \caption{Examples of numerical solutions of the system of ODEs in Equation (%
\ref{}) with different values of $q.$ In
...
\begin{figure}[tbp]
 \centering%
 \includegraphics[width=0.6\textwidth]{figs/fig_ODEsimple_tfpFR}
...
\vspace{1.3cm}
 ...
 The seminal example is due to one founding father of statistics --quote  Francis Galton. At the weight-judging competition at the West of England Fat Stock and Poultry Exhibition he collected 787 cards on which competitors estimated the weight of the meat of an ox after it has been slaughtered and dressed. 
...
\end{table}
 ...
 \section{Quantifying the degree of crowd wisdom}
...
\paragraph{On social influence and the wisdom of the crowd.}
 ...
 \section{How many people are wiser than the crowd?}
 ...
 The questions underlying the two ideas are:
 \begin{compactenum}
 \item How many people are wiser than the crowd?
 \item Can we be better than random sampling by focussing on diversity or conformity while composing the guestimation team?
 \end{compactenum}
...
\end{figure}
 ...
 Consequently, the number of experts increase linearly for large $k$. 
...
\end{figure}
 ...
 Let us finally look at the three emprical samples Galton, Viertelfest, and Oldenburg. 
...
K <- c(1,20,200)
Knames=c("Wk1"="1 draws", "Wk20"="Median 20 iid draws", "Wk200"="Median 200 iid draws")
E <- data_frame(G)
...
g1 <- ggplot(filter(E,G>1080,G<1320), aes(G, weight=weight, alpha=0.8)) + 
  ...
  ggtitle("Galton") + xlab("Estimates (axis clipped)")
...
\paragraph{Diversity and conformity sampling of small and medium sized crowds}

 In the previous paragraph we saw that whenever there is a small bias in the population, there is an optimal crowd size for the crowd to compete optimally with single individuals. 
 This paragraph deals with process of sampling a crowd. 
 ...
 In particular, we will analyze the impact of two sampling strategies: sampling for \emph{diversity} and sampling for \emph{conformity}.
 
 Let us formulate both strategies in an ad hoc but precise way. The full code is available in the source files of this work \parencite{Lorenz2016Sourcefilesand}. Both samples operate in the same way and differ only at one particular point. 
 Let us think of iterative sampling of $x_1, x_2, \dots$ as independent draws from the same sample one after the other.
 Let us call $\bar{x}_i$ be the aggregation of all sampled estimates up to estimate $i$. We first consider this aggregtion to be the mean and discuss the median later. Let us further call $\text{SEM}(x_1,\dots,x_i) = \frac{s_i}{\sqrt{i}}$ be the \emph{standard error of the mean} of the first $i$ estimates, where $s_i$ is the \emph{sample standard deviation}\footnote{$s_i=\sqrt{\frac{1}{i}\sum_{j=1}^i(x_j-\bar{x}_i)^2}$}. 
 ...
 \item For $i>2$, sample estimate $x_i$ such that \\
 $|x_i - \bar{x}_{i-1}| > \text{SEM}(x_1,\dots,x_{i-1})$ for \emph{diversity sampling}, or \\
 $|x_i - \bar{x}_{i-1}| < \text{SEM}(x_1,\dots,x_{i-1})$ for \emph{conformity sampling}. 
 \end{compactenum}
 ...
 Conformity sampling instead becomes more and more restrictive, which makes it more and more difficult to draw 

\begin{figure}
<<DiversitySampling, echo=FALSE>>=
source("R/diversitysampling.r")
arrange_test <- function(test,truth) {
  gather(test, key=key, value=value, diversity, random, conformity) %>% 
    mutate(collErr = collError(value, truth, exponent=1), 
           key = factor(key,levels = c("conformity","random","diversity")))
}
...
@
\caption{Boxplots and mean of (unsquared) collective errors of collective estimates of simulated samples of small crowds ($k=5,10,20,50$) each drawn from the standard normal distribution using conformity, random, and diversity sampling. Crowds' collective estimates aggregated by the mean and the median. For each point \Sexpr{format(n,big.mark=",")} crowds have been sampled. 99.5\% confidence intervals are shown. Sometimes they lie within the area of the points. } \label{fig:DiversitySampling}
\end{figure}

Diversity sampling seems to work (reduces error) for 
very small bias and not very large samples when the aggregation function of the sample is the mean
It seems to have bad effects when your aggregation function is the median!!
 ...
 The wisdom of the crowd effect appears surprising to most individuals although it is a natural statistical property when we see individual estimates as independent random draws a distribution which is unbiased in the sense that the aggregate measure coincides with the correct value. 
...
\section{How majoritarian democracy undermines collective truth-finding.}
\textcite{Lorenz.Rauhut.ea2015MajoritarianDemocracyUndermines}

The interplay of decision rules and communcation in collective judgment tasks have an effect on the chances for bias correction or bias increase on the group level. 


Report Confidence effect also for \parencite{Lorenz.Rauhut.ea2011HowSocialInfluence}

\textcite{Granovskiy.Gold.ea2015IntegrationSocialInformation}


\section{Conclusions from the wisdom of crowds}
