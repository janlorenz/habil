...
@
\caption{Histogram of estimates for the weight-judging competition at the West of England Fat Stock and Poultry Exhibition in Plymouth 1907 ($N=787$). }\label{fig:galton}
\end{figure}
...
@
\caption{Histogram of estimates for the guestimation task ``How many lots will be sold at the end of the festival'' at the Viertelfest Aug 22-24, 2008 in Bremen. ($N=\Sexpr{format(nrow(D), big.mark=",")}$, x-axis clipped at 150,000)}\label{fig:viertelfest}
\end{figure}
...
\paragraph{On social influence and the wisdom of the crowd.}
 ...
 This section presents two ideas how one can derive optimal sizes of guestimation teams and how one should sample them based on the statistical the conception of crowd wisdom. 
...
\end{figure}
 ...
 For a sample of $n$ numbers it is relatively straightforward, to compute a weight vector of the same size where each weight represents the probabilty that this value turns out to be the median of $k$ independent and identically distributed draws from the sample with replacement.\footnote{The computation is documented in \textcite{Lorenz2016Sourcefilesand}.}
...
\section{Diversity and conformity sampling of small medium sized crowds}

<<DiversitySampling, echo=FALSE>>=
# source("R/diversitysampling.r")
# groupsize = 10
# 
# test <- make_n_Samples(G,groupsize,1000,aggregation.fun = "median", replace=FALSE)
# 
# TEST <- gather(test)
# TEST$collErr <- collError(TEST$value,t, exponent=1)
# ggplot(TEST,aes(collErr, color=key)) + geom_histogram(position="identity", alpha = 0) # + facet_grid(key ~ .)
# TEST %>% group_by(key) %>% summarize(mean(collErr), sd(collErr))
# t.test(collError(test$diversity,t),collError(test$normal,t))
@
...
% Pictures of whole range and core range. 

% WoC is a systemic effect of the right choice of aggregation, no bias, and large dispersion
